...
import { AppealCaseFilters } from "./types";
import { AppealFilter } from "./appeal-table-filter/appeal-table-filter";
...
export const AppealTable: React.FC<AppealTableProps> = ({
  caseType,
  cases,
  total,
  pagesCount,
  ...
  } = useTableControls<AppealCaseFilters>({
    initialValues: {
      filters: {
        storeNamesFilter: {
  ...
  const fetchCases = useCallback(async () => {
    const serializedFilters = serializeFilters<AppealFilterParams>(filters);
    await casesService.fetchAppealCases({
      size,
      page,
      sort,
      search,
  ...
  const handleCaseDeleteClick = useCallback((caseData) => {
    dialogManagerService.openDialog(DialogType.DELETE_CASE, {
      caseType: CaseStringType.Appeal,
      caseData,
      handleRefetch: () => {
        setRequestRequired(true);
      },
    ...
    <div className={styles.PageWrapper} id="cases-page">
      ...
      <div className="page-content cases-page">
        <Table
          ...
          title={`${getTableLabelByTab(tab)} Appeals`}
          count={total}
          currentPage={page}
