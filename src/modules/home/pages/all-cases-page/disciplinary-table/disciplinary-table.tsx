...
import styles from "./disciplinary-table.module.scss";
...
import { DisciplinaryFilter } from "./disciplinary-table-filter/disciplinary-table-filter";
...
export const DisciplinaryTable: React.FC<DisciplinaryTableProps> = ({
  caseType,
  cases,
  total,
  pagesCount,
  ...
  } = useTableControls<DisciplinaryCaseFilters>({
    initialValues: {
      filters: {
        storeNamesFilter: {
        ...
        discipliningDateFilter: {
          mapParamsProp: "discipliningDate",
          appliedFilters: [],
        },
      },
    },
  ...
  const fetchCases = useCallback(async () => {
    const serializedFilters =
      serializeFilters<DisciplinaryFilterParams>(filters);
    await casesService.fetchDisciplinaryCases({
      size,
      page,
      sort,
      search,
  ...
  const handleCaseDeleteClick = useCallback((caseData) => {
    dialogManagerService.openDialog(DialogType.DELETE_CASE, {
      caseType: CaseStringType.Disciplinary,
      caseData,
      handleRefetch: () => {
        setRequestRequired(true);
      },
    ...
    <div className={styles.PageWrapper} id="cases-page">
      ...
      <div className="page-content cases-page">
        <Table
          ...
          title={`${getTableLabelByTab(tab)} Disciplinaries`}
          count={total}
          currentPage={page}
